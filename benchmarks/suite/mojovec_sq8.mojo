...
def main() raises:
    ...
    print("Loading data...")
    var db_data = load_bin_data("benchmarks/data/sift1m/sift_base.fvecs")
    var queries_data = load_bin_data(
        "benchmarks/data/sift1m/sift_query.fvecs"
    )
    var gt_data = load_bin_data(
    ...
    var collection = client.create_collection(
        "bench_hnsw", dimension=d, M=M, ef_construction=efConstruction
    )
