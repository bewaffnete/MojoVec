...
struct CollectionIVFPQ(Movable):
    ...
    def load(path: String) raises -> CollectionIVFPQ:
        ...
        # Create a dummy collection. We will overwrite the inner ivfpq.
        # But we need nlist, M etc. Wait, we don't know them yet!
        # It's better to read them from the file manually or just let read_index_ivf_pq allocate it.
