...
@always_inline
def _alloc_aligned(count: Int) -> Pointer[Float32, MutUntrackedOrigin]:
    var bytes = count * 4
    var aligned_bytes = (bytes + 63) // 64 * 64
    var ptr_int = external_call["aligned_alloc", Int](64, aligned_bytes)
    return Pointer[Float32, MutUntrackedOrigin](unsafe_from_address=ptr_int)

@always_inline
def _free_aligned(ptr: Pointer[Float32, MutUntrackedOrigin]):
    if Int(ptr) != 0:
...
struct FlatDistanceComputer(DistanceComputerTrait):
    ...
    var codes: Pointer[Float32, MutUntrackedOrigin]
    ...
    def __init__(
        ...
        codes: Pointer[Float32, MutUntrackedOrigin],
    ...
    def distance(self, id: Int, threshold: Float32 = Float32.MAX) -> Float32:
        ...
        var db_ptr = self.codes.unsafe_offset(id * self.d)
    ...
    def distance_batch4(self, id0: Int, id1: Int, id2: Int, id3: Int) -> InlineArray[Float32, 4]:
        ...
        var ptr0 = self.codes.unsafe_offset(id0 * self.d)
        var ptr1 = self.codes.unsafe_offset(id1 * self.d)
        var ptr2 = self.codes.unsafe_offset(id2 * self.d)
        var ptr3 = self.codes.unsafe_offset(id3 * self.d)
        if self.metric_type == METRIC_L2:
    ...
    def symmetric_distance(self, i: Int, j: Int) -> Float32:
        ...
        var ptr_i = self.codes.unsafe_offset(i * self.d)
        var ptr_j = self.codes.unsafe_offset(j * self.d)
        if self.metric_type == METRIC_L2:
    ...
    def prefetch_vector(self, id: Int):
        ...
        var ptr = self.codes.unsafe_offset(id * self.d)
...
struct IndexFlat(Index, StorageTrait, QuantizerTrait, Movable):
    ...
    def add(mut self, x: Span[Float32, _]):
        ...
        if new_ntotal > self.capacity:
            ...
            if self.ntotal > 0:
                unsafe_memcpy(dest=new_codes, src=self.codes, count=self.ntotal * self.d)
            _free_aligned(self.codes)
            self.codes = new_codes
            self.capacity = new_capacity
            
        var offset = self.ntotal * self.d
        unsafe_memcpy(dest=self.codes.unsafe_offset(offset), src=x_ptr, count=n * self.d)
    ...
    def get_vector(self, id: Int) -> Pointer[Float32, MutUntrackedOrigin]:
        ...
        return self.codes.unsafe_offset(id * self.d)
    ...
    def get_vector_span(self, id: Int) -> Span[Float32, MutUntrackedOrigin]:
        ...
        return Span[Float32, MutUntrackedOrigin](
            unsafe_ptr=self.codes.unsafe_offset(id * self.d), length=self.d
        )
    ...
    def _search_impl[HAS_FILTER: Bool](self, x: Span[Float32, _], k: Int, mut distances: Span[mut=True, Float32, _], mut labels: Span[mut=True, Int, _], filter: Span[UInt8, _]):
        ...
        def process_query(i: Int):
            ...
            var query_ptr = x_ptr.unsafe_offset(query_offset)

            var res_dist_ptr = dist_ptr.unsafe_offset(i * k)
            ...
            for j in range(self_ntotal):
                comptime if HAS_FILTER:
                    if filter_ptr[unsafe_offset=j] > 0:
                        continue
                var db_ptr = self_codes.unsafe_offset(j * self_d)
                var dist: Float32
