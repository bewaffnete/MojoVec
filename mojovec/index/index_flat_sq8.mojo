...
struct SQ8DistanceComputer(DistanceComputerTrait):
    ...
    var codes_u8: Pointer[UInt8, MutUntrackedOrigin]
    var norms_u32: Pointer[UInt32, MutUntrackedOrigin]
    var query_u8: Pointer[UInt8, MutUntrackedOrigin]
    ...
    def __init__(
        ...
        codes_u8: Pointer[UInt8, MutUntrackedOrigin],
        norms_u32: Pointer[UInt32, MutUntrackedOrigin],
        query_u8: Pointer[UInt8, MutUntrackedOrigin],
    ...
    def distance_batch4(self, id0: Int, id1: Int, id2: Int, id3: Int) -> InlineArray[Float32, 4]:
        ...
        if self.metric_type == METRIC_L2:
            ...
            var dots = sq8_signed_dot_product_simd_batch4(
                self.codes_u8.unsafe_offset(id0 * self.d).unsafe_bitcast[Int8](),
                self.codes_u8.unsafe_offset(id1 * self.d).unsafe_bitcast[Int8](),
                self.codes_u8.unsafe_offset(id2 * self.d).unsafe_bitcast[Int8](),
                self.codes_u8.unsafe_offset(id3 * self.d).unsafe_bitcast[Int8](),
                self.query_u8.unsafe_bitcast[Int8](),
                self.d,
            )
    ...
    def _distance_l2(self, id: Int) -> Float32:
        return _sq8_code_distance[False](
            self.query_u8,
            self.codes_u8.unsafe_offset(id * self.d),
            self.query_norm_u32,
            self.norms_u32[unsafe_offset=id],
            self.d,
            self.scale_sq,
        )
    ...
    def symmetric_distance(self, i: Int, j: Int) -> Float32:
        ...
        """
        var ptr_i = self.codes_u8.unsafe_offset(i * self.d)
        var ptr_j = self.codes_u8.unsafe_offset(j * self.d)
        if self.metric_type == METRIC_L2:
            return _sq8_code_distance[False](
                ptr_i,
                ptr_j,
                self.norms_u32[unsafe_offset=i],
                self.norms_u32[unsafe_offset=j],
                self.d,
                self.scale_sq,
            )
        else:
    ...
    def prefetch_vector(self, id: Int):
        ...
        """
        var ptr = self.codes_u8.unsafe_offset(id * self.d)
...
struct IndexFlatSQ8(Index, StorageTrait, QuantizerTrait, Movable):
    ...
    var ntotal: Int
    var metric_type: MetricType
    var codes_f32: Pointer[Float32, MutUntrackedOrigin]
    var codes_u8: Pointer[UInt8, MutUntrackedOrigin]
    var norms_u32: Pointer[UInt32, MutUntrackedOrigin]
    var capacity: Int
    ...
    def __init__(
        ...
        self.codes_f32 = unsafe_alloc[Float32](max(self.capacity * self.d, 1))
        self.codes_u8 = unsafe_alloc[UInt8](max(self.capacity * self.d, 1))
        self.norms_u32 = unsafe_alloc[UInt32](max(self.capacity, 1))
    ...
    def __deinit__(deinit self):
        ...
        if not self._mapping.is_active():
            if Int(self.codes_f32) != 0:
                self.codes_f32.unsafe_free()
            if Int(self.codes_u8) != 0:
                self.codes_u8.unsafe_free()
            if Int(self.norms_u32) != 0:
                self.norms_u32.unsafe_free()
        self._mapping.close()
    ...
    def _detach_mapped(mut self):
        if not self._mapping.is_active():
            return
        var new_f32 = unsafe_alloc[Float32](max(self.capacity * self.d, 1))
        var new_u8 = unsafe_alloc[UInt8](max(self.capacity * self.d, 1))
        var new_norms = unsafe_alloc[UInt32](max(self.capacity, 1))
        for i in range(self.ntotal * self.d):
            new_f32[unsafe_offset=i] = self.codes_f32[unsafe_offset=i]
            new_u8[unsafe_offset=i] = self.codes_u8[unsafe_offset=i]
        for i in range(self.ntotal):
            new_norms[unsafe_offset=i] = self.norms_u32[unsafe_offset=i]
        self._mapping.close()
    ...
    def add(mut self, x: Span[Float32, _]):
        ...
        if self.ntotal + n > self.capacity:
            ...
            var new_f32 = unsafe_alloc[Float32](new_cap * self.d)
            var new_u8 = unsafe_alloc[UInt8](new_cap * self.d)
            var new_norms = unsafe_alloc[UInt32](new_cap)
        ...
        unsafe_memcpy(
            dest=self.codes_f32.unsafe_offset(offset_f32),
            src=x_ptr,
            count=n * self.d,
        )
        if self.metric_type == METRIC_L2:
            _encode_sq8_batch[False](
                x_ptr,
                self.codes_u8.unsafe_offset(offset_f32),
                self.norms_u32.unsafe_offset(self.ntotal),
                n,
                self.d,
                self.global_min,
                inverse_scale,
        ...
        def process_query(i: Int):
            ...
            for j in range(self_ntotal):
                comptime if HAS_FILTER:
                    if filter_ptr[unsafe_offset=j] > 0:
                        continue
                var dist: Float32
                if self_metric_type == METRIC_L2:
                    dist = _sq8_code_distance[False](
                        query_u8,
                        self_codes_u8.unsafe_offset(j * self_d),
                        query_norm,
                        self_norms_u32[unsafe_offset=j],
                        self_d,
                        scale_sq,
                    )
                else:
            ...
            var sorted_dist_ptr = distances_ptr.unsafe_offset(i * k)
            var sorted_labels_ptr = labels_ptr.unsafe_offset(i * k)
            ...
            while heap_size > 0:
                ...
                heap_size -= 1
                var idx = heap_size
                sorted_dist_ptr[unsafe_offset=idx] = popped.dist
                sorted_labels_ptr[unsafe_offset=idx] = Int(popped.label)
                
            for j in range(result_count, k):
                sorted_dist_ptr[unsafe_offset=j] = 0.0
                sorted_labels_ptr[unsafe_offset=j] = -1
                
            if self_metric_type == METRIC_INNER_PRODUCT:
                for j in range(result_count):
                    if res_labels_ptr[unsafe_offset=j] >= 0:
                        res_dist_ptr[unsafe_offset=j] = -res_dist_ptr[unsafe_offset=j]
                    
            query_u8.unsafe_free()
    ...
    def get_distance_computer(self, query: Pointer[Float32, _]) -> Self.ComputerType:
        ...
        var query_u8 = unsafe_alloc[UInt8](self.d)
        var inverse_scale = 1.0 / self.scale
        var query_norm: UInt32
        if self.metric_type == METRIC_L2:
            query_norm = _encode_sq8_vector[False](
    ...
    def get_vector(self, id: Int) -> Pointer[Float32, MutUntrackedOrigin]:
        ...
        """
        return self.codes_f32.unsafe_offset(id * self.d)
    ...
    def get_vector_span(self, id: Int) -> Span[Float32, MutUntrackedOrigin]:
        ...
        return Span[Float32, MutUntrackedOrigin](
            unsafe_ptr=self.codes_f32.unsafe_offset(id * self.d), length=self.d
        )
