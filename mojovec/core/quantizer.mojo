...
struct ScalarQuantizer(Copyable):
    ...
    def train(mut self, x: Span[Float32, _]):
        ...
        for j in range(self.d):
            vmin_ptr[unsafe_offset=j] = x_data[unsafe_offset=j]
            vmax_ptr[unsafe_offset=j] = x_data[unsafe_offset=j]

        for i in range(1, n):
            var x_ptr = x_data.unsafe_offset(i * self.d)
            for j in range(self.d):
                if x_ptr[unsafe_offset=j] < vmin_ptr[unsafe_offset=j]:
                    vmin_ptr[unsafe_offset=j] = x_ptr[unsafe_offset=j]
                if x_ptr[unsafe_offset=j] > vmax_ptr[unsafe_offset=j]:
                    vmax_ptr[unsafe_offset=j] = x_ptr[unsafe_offset=j]

        for i in range(self.d):
            vdiff_ptr[unsafe_offset=i] = vmax_ptr[unsafe_offset=i] - vmin_ptr[unsafe_offset=i]

        self.is_trained = True

    def encode(self, x: Pointer[Float32, _], codes: Pointer[UInt8, MutUntrackedOrigin]):
        """
        ...
        """
        if self.qtype == QT_fp16:
            var codes_fp16 = codes.unsafe_bitcast[Float16]()
            for i in range(self.d):
                codes_fp16[unsafe_offset=i] = x[unsafe_offset=i].cast[DType.float16]()
        elif self.qtype == QT_8bit:
            var vmin_ptr = self._calibration[].vmin.unsafe_ptr()
            var vdiff_ptr = self._calibration[].vdiff.unsafe_ptr()
            for i in range(self.d):
                var vdiff_safe = vdiff_ptr[unsafe_offset=i]
                if vdiff_safe == 0.0:
                    codes[unsafe_offset=i] = 0
                else:
                    var xi = (x[unsafe_offset=i] - vmin_ptr[unsafe_offset=i]) / vdiff_safe
                    if xi < 0.0:
                        codes[unsafe_offset=i] = 0
                    elif xi > 1.0:
                        codes[unsafe_offset=i] = 255
                    else:
                        codes[unsafe_offset=i] = (xi * 255.0 + 0.5).cast[DType.uint8]()

    def decode(self, codes: Pointer[UInt8, MutUntrackedOrigin], x: Pointer[Float32, MutUntrackedOrigin]):
        """
        ...
        if self.qtype == QT_fp16:
            var codes_fp16 = codes.unsafe_bitcast[Float16]()
            for i in range(self.d):
                x[unsafe_offset=i] = codes_fp16[unsafe_offset=i].cast[DType.float32]()
        elif self.qtype == QT_8bit:
            var vmin_ptr = self._calibration[].vmin.unsafe_ptr()
            var vdiff_ptr = self._calibration[].vdiff.unsafe_ptr()
            for i in range(self.d):
                var xi = codes[unsafe_offset=i].cast[DType.float32]() / 255.0
                x[unsafe_offset=i] = xi * vdiff_ptr[unsafe_offset=i] + vmin_ptr[unsafe_offset=i]
